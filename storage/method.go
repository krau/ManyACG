...
// 保存图片的所有尺寸
func SaveAll(ctx context.Context, artwork *entity.Artwork, picture *entity.Picture) (*shared.StorageInfo, error) {
	// if len(Storages) == 0 {
	...
	// var regularDetail *shared.StorageDetail
	...
	// 	}
	// }
	// var thumbDetail *shared.StorageDetail
...
func Save(ctx context.Context, filePath string, storagePath string, storageType shared.StorageType) (*shared.StorageDetail, error) {
...
func GetFile(ctx context.Context, detail *shared.StorageDetail) ([]byte, error) {
...
func Delete(ctx context.Context, info *shared.StorageDetail) error {
...
func DeleteAll(ctx context.Context, info shared.StorageInfo) error {
	// var wg sync.WaitGroup
	// ctx, cancel := context.WithCancel(ctx)
	// defer cancel()
	// errChan := make(chan error)
	// for _, detail := range []*shared.StorageDetail{info.Original, info.Regular, info.Thumb} {
	// 	if detail == nil {
	// 		continue
	// 	}
	// 	wg.Add(1)
	// 	go func(detail *shared.StorageDetail) {
	// 		defer wg.Done()
...
func ServeFile(ctx *gin.Context, detail *shared.StorageDetail) {
...
// func GetFileStream(ctx context.Context, detail *shared.StorageDetail) (io.ReadCloser, error) {
// 	if detail == nil {
