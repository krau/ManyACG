...
func distributeOctTree(keys []keyPoint, minX, maxX, minY, maxY, n int) []keyPoint {
	...
	// Iteratively subdivide multi-key nodes until we have at least N nodes or
	// no further subdivision is possible. When a single round would overshoot
	// N, subdivide the densest nodes first (they benefit most), matching the
	// spirit of ORB-SLAM3's DistributeOctTree.
	for {
		prevSize := len(nodes)
