...
// Distance returns the Hamming distance between two equal-length byte slices.
...
// KNN returns the k candidates in vb with the smallest Hamming distance to va.
func KNN(va []byte, vb [][]byte, k int) []Neighbor {
	...
	// Bounded max-heap keyed by distance: the root holds the largest distance
	// among the current best k, so it can be evicted when a closer candidate
	// appears. This keeps the k smallest distances overall.
	h := make([]Neighbor, 0, k)
	for i, v := range vb {
		d := Distance(va, v)
		if len(h) < k {
