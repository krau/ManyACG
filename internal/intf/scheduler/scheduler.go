...
// 	artworkCh := make(chan *types.Artwork, config.Get().Fetcher.MaxConcurrent)
...
// 			}
// 		}(source, config.Get().Fetcher.Limit, artworkCh, source.Config().Intervel)
// 	}
...
// 			err := telegram.PostAndCreateArtwork(ctx, artwork, telegram.Bot, config.Get().Telegram.Admins[0], 0)
// 			if err != nil {
...
// 			}
// 			time.Sleep(time.Duration(int(config.Get().Telegram.Sleep)*len(artwork.Pictures)) * time.Second)
// 		} else {
